...
<form method="post" action="panel/a_alq.php?id=<?php echo $id;?>&vi=<?php echo $vi;?>" id="frmRegistro" name="frmRegistro">
...
<?php if($_SESSION["tipo"] == 'admin'){?>
 ...
 <?php    };  ?> 
    ...
    <div class="campb margin-bottom">
      ...
      <input type="text" name="fecha_renta" class="inputgral2" placeholder=""  id="datetimepicker" 
      value="<?php 
                    if($_REQUEST["id"]!='' && $row["fecha_renta"]!='0000-00-00 00:00:00'){
                         echo substr($row["fecha_renta"],8,2).'/'.substr($row["fecha_renta"],5,2);
                         echo '/'.substr($row["fecha_renta"],0,4);
                         echo ' '.substr($row["fecha_renta"],11,5);
                    };

                    if($id=='') {
                      echo substr($fecha_hoy,8,2).'/'.substr($fecha_hoy,5,2);
                       echo '/'.substr($fecha_hoy,0,4);
                       echo ' '.substr($fecha_hoy,11,5);
                     };
              ?>"/>
    </div>
...
<?php if ($id != ''){?>
    <div class="campb dospor margin-bottom" id="fecha_deb">
      ...
      <input type="text" name="fecha_devolucion" class="inputgral2" placeholder="Fecha de Devolucion" id="datetimepicker2"
       value="<?php if($row["fecha_devolucion"]!= '0000-00-00 00:00:00' && $row["fecha_devolucion"]!= ''){ 
                   echo substr($row["fecha_devolucion"],8,2).'/'.substr($row["fecha_renta"],5,2);
                   echo '/'.substr($row["fecha_devolucion"],0,4);
                   echo ' '.substr($row["fecha_devolucion"],11,5);
                 };
            ?>"/>
    </div>
<?php };?>
  ...
  <?php
    //$fecha_hoy
    if($row["fecha_devolucion"]=='0000-00-00 00:00:00' || $row["fecha_devolucion"]==''){
      $lo_devolvio = 'no';
    }else{
      $lo_devolvio = 'si';
    };
    
    $alquiler_mas_dos = strtotime ( '+2 hour' , strtotime ( $fecha_hoy ) );
    $alquiler_mas_dos = date ( 'Y-m-d H:i:s' , $fecha_hoy );

    if($alquiler_mas_dos< $fecha_hoy && $lo_devolvio == 'no'){
          //$datetime1 = date_create(substr($row["fecha_renta"],6,4));
          $fecha1 = substr($row["fecha_alquilada"],0,10);
          $fecha2 = substr($fecha_hoy,0,10);
          //$fecha1 = "2009-10-11";
          //$fecha2 = "2009-10-13";
          $datetime1 = date_create($fecha1);
          $datetime2 = date_create($fecha2);
          $interval = date_diff($datetime1, $datetime2);//saco la cantidad de dias de retraso
          $sss = $interval->format('%a');
          if($sss == '0'){$sss = 1;$bandera_sss = 1;};
          $sql9h= "SELECT * FROM remolques WHERE id='".$row["remolque"]."' ORDER BY numero ASC";
          $result9h = mysql_query($sql9h, $conn1);
          $row9h = mysql_fetch_array($result9h);

          $cantidad_mas = $row9h["valor_dia"] * $sss;// saco la cantidad de dinero por dias extras
          $valor_t = $row["precio_total"] + $cantidad_mas;      
    };
  ?>
  <div class="campb dosporch">
  <?php if($row["fecha_renta"]!='' && $row["fecha_renta"]!='0000-00-00 00:00:00') {?>
      <div>
          Fecha de Regreso: 
          <div id="regreso"><?php echo $row["fecha_alquilada"];?></div>
      </div>
<?php };  ?>
      <div id="preciodecompra">
        <?php 
         if($row["fecha_renta"]!='' && $row["fecha_renta"]!='0000-00-00 00:00:00') {
                echo '$'.$row["precio_total"];
                if($sss != ''){    echo ' + <span style="color:#FF0000;">$'. $cantidad_mas.' </span> = $'.$valor_t.'<br>';    };
                //echo '<br>'.$row9h["valor_dia"].' * '.$sss;
        }elseif($id != ''){echo '<span style="font-size:14px;">Para generar el calulo se tiene ingresar una fecha de 
        renta correcta</span>';};
        ?>
      </div>
      <div><?php if($row["fecha_renta"]!='' && $row["fecha_renta"]!='0000-00-00 00:00:00') {
      if($sss != ''){    
        echo $sss.' dias de retraso = $'.$cantidad_mas;    
        if ($bandera_sss == '1') {
          echo " (Supero las 2hs de tolerancia)";
        }
      };};
      ?> </div>
...
<div class="campitem margin-bottom2">
  ...
  <div class="save-btns">
      <div class="saveaddprint" onclick="guardareimprimir()">Guardar e <br>imprimir</div>
...
function guardareimprimir(){
  document.getElementById("imprimir").value = 'si';
...
<?php //*?/?>
  ...
  <script type="text/javascript" src="jquery/moment/min/moment.min.js"></script>      
  <script type="text/javascript" src="jquery/bootstrap/dist/js/bootstrap.js"></script>
  <script type="text/javascript" src="jquery/build/js/bootstrap-datetimepicker.min.js"></script>
  <link rel="stylesheet" href="jquery/bootstrap/dist/css/bootstrap.min.css" />
  <link rel="stylesheet" href="jquery/build/css/bootstrap-datetimepicker.css" />
...
$(function () {
            ...
            $('#datetimepicker2').datetimepicker({
                ...
                defaultDate: "<?php 
                if($row["fecha_devolucion"]!= '0000-00-00 00:00:00'  && $row["fecha_devolucion"]!= ''){
                   echo substr($row["fecha_devolucion"],8,2).'/'.substr($row["fecha_devolucion"],5,2);
                   echo '/'.substr($row["fecha_devolucion"],0,4);
                   echo ' '.substr($row["fecha_devolucion"],11,5);
                };?>"
            });
