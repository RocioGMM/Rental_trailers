...
<div class="cf"></div>
  ...
  <div class="campitem-cliente">    
    ...
    <?php };?>
    <input type="hidden" name="cant_producto" value="<?php echo $total_productos;?>"/>
  </div>
<!--
  <div class="campitem-cliente">    
    <div class="campb ">
  ...
  <div class="campitem-cliente">
    ...
    <div class="campb">
      ...
      if($_SESSION["tipo"] == 'admin'){
      	...
      	$sql9h= "SELECT * FROM cliente WHERE vendedor='".$_SESSION["vendedor"]."' || vendedor='' ORDER BY nombre_empresa ASC";
      };    //*/    
      	$sql9h= "SELECT * FROM cliente ORDER BY nombre_empresa ASC";
...
<?php if($_SESSION["tipo"] == 'admin'){?>
  <div class="campitem-cliente">
    ...
    <span class="grin">*Debes programar una tarea para guardar la venta.</span>
    </div>
...
<div class="campe" >    
	          <span class="lbelbg">Cierre sin Compra</span> 
	          <label for="textfield"></label>
	          <input name="cerrarforz" type="checkbox" value="1" style="margin-left: 11px;"  id="cerrarforz"/>
	    </div>


<span class="titlecol tcolor" >PROGRESO
<?php
if($id == ''){$total_registrosact = '0';}
else{
...
<div class="t-progres-edit">
    <?php 
    	...
    	$sql5 = "SELECT * FROM evento_act WHERE evento_cat ='2' AND evento='".$row["id"]."' AND checkedeve='1' ORDER BY id DESC";// Visita
		$result5 = mysql_query($sql5, $conn1);
		$row5 = mysql_fetch_array($result5); 
		if($row5["id"] == ''){	$iconocas = 'griscasa.png';		}
		else{	$iconocas = 'casaicon.png';	};

    	/*$sql5 = "SELECT * FROM evento_act WHERE evento_cat ='5' AND evento='".$row["id"]."' AND checkedeve='1' ORDER BY id DESC";// diagnostico
		$result5 = mysql_query($sql5, $conn1);
		$row5 = mysql_fetch_array($result5); 
		if($row5["id"] == ''){	$iconodia = 'grisdiagnostico.png';		}
		else{	$iconodia = 'diagnostico.png';	};//*/

    	$sql5 = "SELECT * FROM evento_act WHERE evento_cat ='3' AND evento='".$row["id"]."' AND checkedeve='1' ORDER BY id DESC";// cotizacion
		$result5 = mysql_query($sql5, $conn1);
		$row5 = mysql_fetch_array($result5); 
		if($row5["id"] == ''){	$iconocot = 'grismonto.png';		}
		else{	$iconocot = 'monto.png';	};

    	$sql5 = "SELECT * FROM evento_act WHERE evento_cat ='6' AND evento='".$row["id"]."' AND checkedeve='1' ORDER BY id DESC";// Muestra
		$result5 = mysql_query($sql5, $conn1);
		$row5 = mysql_fetch_array($result5); 
		if($row5["id"] == ''){	$iconomues = 'grisflechaabajo.png';		}
		else{	$iconomues = 'flechaabajo.png';	};

    	$sql5 = "SELECT * FROM evento_act WHERE evento_cat ='4' AND evento='".$row["id"]."' AND checkedeve='1' ORDER BY id DESC";// Cierre
		$result5 = mysql_query($sql5, $conn1);
		$row5 = mysql_fetch_array($result5); 
...
//	if($id == ''){	$sql3 = "SELECT * FROM evento_cat ORDER BY id ASC";		}
	...
	if($id != ''){
		$total_regi = 0;
		$sql2c = "SELECT * FROM evento_act WHERE evento='".$row["id"]."' AND evento_cat='".$row3["id"]."' AND checkedeve='1' ORDER BY id DESC";
		$result2c = mysql_query($sql2c, $conn1);
		$total_regi = mysql_num_rows($result2c);
		$row2c = mysql_fetch_array($result2c);

		
		
		$sql2 = "SELECT * FROM evento_act WHERE evento='".$row["id"]."' AND evento_cat='".$row3["id"]."' ORDER BY id DESC";
		$result2 = mysql_query($sql2, $conn1);	
//		$total_regi = mysql_num_rows($result2);	
...
?>
  <div class="campitem" 
  <?php if ($row3["id"] == 1){echo 'id="muescone"';}	elseif ($row3["id"] == 2){echo 'id="muescone2"';}	elseif ($row3["id"] == 6){echo 'id="muescone3"';};?>>
...
<?php //if($id != ''){	
	...
	<a data-fancybox-type="iframe" href="historial.php?id=<?php echo $row["id"];?>&evento_cat=<?php echo $row3["id"];?>" 
		...
		<span class="date-hist">Última: <?php echo substr($row2c["fecha"],8,2).' / '.replacemesshort(substr($row2c["fecha"],5,2)).' - '.substr($row2c["fecha"],11,5);?></span></span>
	</a>
	<?php };	?>

<?php if($row3["id"] == '1'){?><div class="addcall-now" id="addotro<?php echo $row3["id"]?>" <?php if($row2["checkedeve"]!="1"){echo 'style="display:none"';};?>>
								<a onclick="clonetel()"><img src="addcall-act.png" width="49" height="30" /></a></div><?php };?>
<?php if($row3["id"] == '2'){?><div class="addcall-now" id="addotro<?php echo $row3["id"]?>" <?php if($row2["checkedeve"]!="1"){echo 'style="display:none"';};?>>
...
<?php 	if($row3["fecha_fin"] == 's'){
		$arrayfechafin[$cantfechafin] = $cantidadeveact;
		$cantfechafin = $cantfechafin + 1;
?>
<div class="horafecha ">
  <div class="campd fechs">
      <label for="textfield"></label>
<input type="text" name="fechafin<?php echo $cantidadeveact?>" id="<?php if ($movil == 'movil'){ echo 'datetimepicker3';	}else {		echo 'datetimepicker2';	};
 echo $cantidadeveact?>f" placeholder="Fecha Fin" 
...
<div class="subirarchivo">
	<?php  if($row2["archivo"] != ''){?><a href="<?php echo 'archivos/'.$row2["archivo"];?>"><img src="cotizacionbajar.png"></a>
	<?php }else{	?>
...
<div class="campitem">
    <div class="campe" >    
<span class="lbelbg" style="margin-top:34px">OBSERVACIONES</span>
    </div>
...
<div class="actions-bar">
  ...
  <span class="indic" id="addt3" style="display:none;max-width:300px;">
	  <a><strong>AGREGAR: </strong></a>
	  <a onclick="clonetel()" class="cloneaction">LLAMADA</a> / 
	  <a onclick="clonevis()" class="cloneaction">VISITA</a>
	  <!--Agregar una tarea nueva significara agregar un campo "tarea" mas para todos los vendedores, si lo que quieres es agregar una visita o una llamada mas, 
	  la opción de agregar en estos campos aparecerá luego de que hagas check en "llamada" o "visita". 
	  Para guardar el proyecto has click en el botón verde "Guardar" en la parte superior 
	  de este formulario.
	  Si no quieres agregar una nueva tarea ignora el boton "NUEVA TAREA".
	  <input type="text" name="tarea"  id="tarea"  placeholder="NUEVA TAREA" class="hospitalx" maxlength="10"/>
	  <a class="addbtn-tarea" onclick="clonetare()" id="addt1"></a>-->
  </span>
  
  <!--
  <a href="agregar-venta.php" class="addbtn-nuevo"></a> 
...
<?php /*		if($id != ''){		?>
<a href="eliminar-evento.php?id=<?php echo $id;?>" class="rax">Elliminar</a>
<?php 		};	//*/	?>
...
<div style="display:none;"><!------------------------------------ 		MUESTRAS DE INGRESA --------------------------------- -->
	<div class="campitem"  id="telcone">
	            
	    <input type="hidden" name="idx" value=""/>
	    <input type="hidden" name="evento_catx" value="1"/>
	    <input type="hidden" name="evento_cat_nombrex" value=""/>

	    <div class="campe" >    
	          <span class="lbelbg">Llamadas*</span> 
	          <label for="textfield"></label>
	          <input name="checkedeveX" type="checkbox" value="1" style="margin-left: 11px;"  id="escuchallla" onchange="escucharidllamada()"/>
	    </div>
	    	 
	    <div class="horafecha">
	      <div class="campd fechs"><label for="textfield"></label>
	      <input type="text" name="fechax" id="datetimepicker2x" placeholder="Fecha" value="" class="hospitalx" />
	    </div>
	     
	    <div class="campd fechs">    
	         <label for="textfield"></label>
	         <input type="text" name="horax" id="datetimepicker1x" placeholder="Hora" value="" class="hospitalx"/>
	    </div></div>
	 

	</div>	
	<div class="campitem"  id="viscone">
	            
	    <input type="hidden" name="idx" value=""/>
	    <input type="hidden" name="evento_catx" value="2"/>
	    <input type="hidden" name="evento_cat_nombrex" value=""/>

	    <div class="campe" >    
	          <span class="lbelbg">Visitas</span> 
	          <label for="textfield"></label>
	          <input name="checkedeveX" type="checkbox" value="1" style="margin-left: 11px;"  id="escuchallvis" onchange="escucharidvisita()"/>
	    </div>
...
</div><!------------------------------------ 	FIN DE MUESTRAS DE INGRESA --------------------------------- -->
...
<script type="text/javascript">
	...
	function clonetel() {
		...
		var segundo_p2 = document.getElementById('muescone');// Despues de quien lo quiero meter
		$('#muescone').append(clonedDiv);
		 ...
		 var padre_f = document.getElementById("muescone").getElementsByTagName("span");//Saco el titulo "programar"
		 //padre_f[4].style.display = "none";
		 var padre_c = document.getElementById("muescone").getElementsByTagName("div");
	...
	function clonevis() {
		...
		clonedDiv.attr("id", "telclone2"); // Cambio ID
		var segundo_p = document.getElementById('lugari');// Despues de quien lo quiero meter
		var segundo_p2 = document.getElementById('muescone2');// Despues de quien lo quiero meter
		$('#muescone2').append(clonedDiv);
		 var porId=document.getElementById('cantidadeveact').value;// Obtenemos el valor de la cantidad actual de elementos por el id del input bandera
		 porId2 = parseInt(porId);
		 var nuevovalor = porId2 + 1;
		 ...
		 var padre_f = document.getElementById("muescone2").getElementsByTagName("span");//Saco el titulo "programar"
		 //padre_f[4].style.display = "none";
		 var padre_c = document.getElementById("muescone2").getElementsByTagName("div");
		 padre_c[1].style.display = "none";//SACO EL AGREGAR LLAMADA	
		 padre_i = padre_c[2].getElementsByTagName("a");
		 padre_i[0].style.display = "none";//SACO icono der, calendario
		 //var padre = document.getElementById("telcone").getElementsByTagName("a");	 
...
//		 segundo_c[1].style.display = "none";//SACO EL AGREGAR LLAMADA		
	...
	function clonetare() {
		...
		var formulario = document.getElementById('tarea');
        ...
        var formulario = document.getElementById('tarea');
        stringtarea = formulario.value;
		
		   for(i=0; i<stringtarea.length; i++){
		      if (numeros.indexOf(stringtarea.charAt(i),0)!=-1){
		         todoCorrecto=false;
		      }
		   }
	if(todoCorrecto == false){		   
		alert ('El nombre de la tarea no puede estar vacía o contener sólo espacios en blanco ni contener numeros');
	}

if(todoCorrecto == true){// VERIFICAR TODO CORRECTO
		var clonedDiv = $('#viscone').clone(); // Clono
		clonedDiv.attr("id", "telclone2"); // Cambio ID
		var segundo_p = document.getElementById('lugari');// Despues de quien lo quiero meter
		var segundo_p2 = document.getElementById('muescone3');// Despues de quien lo quiero meter
		$('#muescone3').append(clonedDiv);
		 var porId=document.getElementById('cantidadeveact').value;// Obtenemos el valor de la cantidad actual de elementos por el id del input bandera
		 porId2 = parseInt(porId);
		 var nuevovalor = porId2 + 1;
		 ...
		 //primero[2].remove();// SACO EL hISTORIAL ICONO DE ALLADO DEL H
		...
		}// FIN DEL IF VERIFICAR TODO CORRECTO
	}// FIN DE LA FUNCION
