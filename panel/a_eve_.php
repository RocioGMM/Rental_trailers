							...
							if (!(strpos($tipo1, "msword")) && !(strpos($tipo1, "pdf")) && !(strpos($tipo1, "x-download")) && !(strpos($tipo1, "octet-stream")) 
								&& !(strpos($tipo1, "zip")) && !(strpos($tipo1, "jpeg")) && !(strpos($tipo1, "x-shockwave-flash")) && ($peso < 20000000)){
									...
									if (move_uploaded_file($_FILES[$archivo]['tmp_name'], "../archivos/".$nombrearch)){
										$sql2 = "INSERT INTO evento_act(evento, evento_cat, fecha, archivo, folio, importe, checkedeve, vendedor, serie) 
										VALUES('".$evento."', '".$evento_cat."', '".$fechaev."', '".$nombrearch."', '".$_POST[$folio]."', '".$_POST[$importe]."', 
											'".$_POST[$checkedeve]."', '".$vendedor."', '".$_POST[$serie]."')";
										$result2 = mysql_query($sql2);
									}else{	$err = '1';	};
							};
						}else{
										$sql2 = "INSERT INTO evento_act(evento, evento_cat, fecha, folio, importe, checkedeve, vendedor, serie) 
							...
							if (!(strpos($tipo1, "msword")) && !(strpos($tipo1, "pdf")) && !(strpos($tipo1, "x-download")) && !(strpos($tipo1, "octet-stream")) 
								&& !(strpos($tipo1, "zip")) && !(strpos($tipo1, "jpeg")) && !(strpos($tipo1, "x-shockwave-flash")) && ($peso < 20000000)){
									...
									if (move_uploaded_file($_FILES[$archivo]['tmp_name'], "../archivos/".$nombrearch)){
										...
										importe='".$_POST[$importe]."', checkedeve='".$_POST[$checkedeve]."', serie='".$_POST[$serie]."' WHERE id=".$_POST[$idi];
										$result2 = mysql_query($sql2);
									}else{	$err = '1';	};
							};
						}else{
										$sql2 = "UPDATE evento_act SET fecha='".$fechaev."', folio='".$_POST[$folio]."', importe='".$_POST[$importe]."', 
										checkedeve='".$_POST[$checkedeve]."', serie='".$_POST[$serie]."' WHERE id=".$_POST[$idi];
										$result2 = mysql_query($sql2);
...
//echo 'yea';
			...
			else{	header("location: ../programacion.php?comentarios=".$comentarios."&SERIE.=".$_POST[$serie1]."&id=".$id);		};
