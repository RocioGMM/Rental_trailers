...
<div class="">
    ...
    <div class="campx" ><?php //echo $movil;?> 
      ...
      <input type="text" name="fechaest1" id="datetimepicker2" placeholder="Fecha Inicio" class="hospitalzz" value="<?php echo substr($_REQUEST["fechaest1"],0,10);?>" />
    </div>
    <span class="indic">A</span>
    <div class="campx" ><?php //echo $movil;?> 
      <label for="textfield"></label>
      <input type="text" name="fechaest2" id="datetimepicker2b" placeholder="Fecha" class="hospitalzz" value="<?php echo substr($_REQUEST["fechaest2"],0,10);?>" />
    </div>
...
<?php };	 $veces_text = 1;	?>
    ...
    <a <?php  if ($_SESSION["tipo"] != 'user'){?> href="agregar-evento.php?id=<?php echo $row["id"];?>"<?php };?> target="_self">  
        <div class="time-s ti"><?php echo substr($row["fecha"],8,2).' '.replacemes(substr($row["fecha"],5,2));?></div>  
    </a>  
...
<div class="ic-idel"><a href="agregar-evento.php?id=<?php echo $row["id"];?>" target="_self"><img src="editaricon.png" width="44" height="44" border="0" /></a></div>
 ...
 <?php //DETECTAR EL ULTIMO EVENTO CHECK
        $sql5 = "SELECT * FROM evento_act WHERE evento='".$row["evento"]."' AND checkedeve ='1' ORDER BY fecha DESC";// Llamada
...
?>
    ...
    <?php 
        $sql5 = "SELECT * FROM evento_act WHERE evento='".$row["evento"]."' AND evento_cat ='6' ORDER BY id DESC";// Cierre
        $result5 = mysql_query($sql5, $conn1);
        $row5 = mysql_fetch_array($result5); 
    ...
    <?php 
        $sql5 = "SELECT * FROM evento_act WHERE evento='".$row["evento"]."' AND evento_cat ='4' ORDER BY id DESC";// Muestra
        $result5 = mysql_query($sql5, $conn1);
        $row5 = mysql_fetch_array($result5); 
