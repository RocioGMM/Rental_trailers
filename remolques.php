...
<div class="onecol"><span class="titlecol-ad">Remolques</span>
<form action="" method="post" class="cuscliform" >
  <input type="text" name="busclie" id="busclie" placeholder=" Buscar Remolque" class="buscli1"/> 
  <input type="submit" name="add"  value="Buscar" class="buscli2"/>
</form>
<div class="icons-men"><a href="agregar-remolques.php"><img src="add-user.png" width="64" height="42" /></a></div>
<div class="divline"></div>
  
  <div class="initem">
    <div class="t-uname">NUMERO </div>
    <div class="t-uemail">MODELO</div>
    <div class="t-contact">CAPACIDAD</div>
    <div class="t-contact">MEDIDAS</div>
...
<?php 
      if($_POST["busclie"] != ''){
        ...
        $sql1 = "SELECT * FROM remolques WHERE numero like '%".$_POST["busclie"]."%' OR serie like '%".$_POST["busclie"]."%' OR placas like '%".$_POST["busclie"]."%' OR capacidad like '%".$_POST["busclie"]."%' OR modelo like '%".$_POST["busclie"]."%' ORDER BY nombre_empresa ASC";
      }else{
        $sql1 = "SELECT * FROM remolques ORDER BY numero ASC";
      };  		
		  $result1 = mysql_query($sql1, $conn1);
		  while($row1 = mysql_fetch_array($result1)){
?>
  <div class="initem"<?php if($row1["vendedor"] ==''){echo 'style="color:#FF0000 !important;"';};?>>
    <div class="uname"><?php echo $row1["numero"];?></div>
    <div class="uemail"><?php echo $row1["modelo"];?></div>
    <div class="contact"><?php echo $row1["capacidad"];?></div>
    <div class="contact"><?php echo $row1["medidas"];?></div>
 
 
<div style="	float: right;	height: 37px;	width: 155px;	margin-top: 2px;	margin-bottom: 1px;"> 
<?php //if($_SESSION["tipo"] == 'admin'){?>
<div class="ic-idel"><a href="eliminar-remolques.php?id=<?php echo $row1["id"];?>" target="_self"><img src="delete1.png" width="44" height="44" border="0" /></a></div>
<?php //};?>
<div class="ic-idel"><a href="agregar-remolques.php?tipo=mod&id=<?php echo $row1["id"];?>"><img src="editaricon.png" width="44" height="44" border="0" /></a></div>
